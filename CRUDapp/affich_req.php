    ...
    if (empty($id)){
      ...
      if ($validId == true){
        try {
          ...
          $req = $connex->prepare('SELECT * FROM cheval WHERE id = :id');
          $req->bindParam (':id', $id);
      	  $req->execute();
      	  $consult = $req->fetch(PDO::FETCH_ASSOC);
          json_encode($consult);
          $resultat = implode(", ", $consult);
          echo "Informations sur le cheval: ",$resultat, "<br><br><br>";
        } catch (\Exception $e) {
      ...
      <a href="consultation.php"> Consulter une donnée </a>
      </div>
